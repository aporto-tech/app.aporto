...
function buildMcpServer(userId: number, authHeader: string, internalBaseUrl?: string, integrationPublicId?: string | null) {
    ...
    async function callSkill(
        ...
        for (let attempt = 1; attempt <= MAX_PROVIDER_ATTEMPTS; attempt++) {
            ...
            if (recordedErrorType === "storage_error" || !shouldRetryProviderFailure(provider, recordedErrorType)) break;
        }
    ...
    server.tool(
        "aporto_run_skill",
        "High-level Aporto skill runner. Use this as the default tool when the user asks you to perform a task. It discovers the skill when needed, selects a provider, executes it, stores a SkillRun, and returns a runId on every response. If status is needs_selection, show the choices to the user once and then call aporto_run_skill again with the chosen skillId. When status is succeeded, return the artifacts to the user. When status is running or waiting, automatically call aporto_get_skill_run every 30 seconds until status is succeeded or failed; do not ask the user to poll manually. When status is failed, report the error to the user and do not automatically rerun aporto_run_skill.",
        {
