...
function buildMcpServer(userId: number, authHeader: string, internalBaseUrl?: string) {
    ...
    server.tool(
        "aporto_execute_skill",
        "Low-level execution for an Aporto skill by ID. This may return a provider taskId for async skills. Prefer aporto_run_skill when you want Aporto to execute, poll, store artifacts, and return the final result when possible.",
        {
            skillId:   z.number().int().describe("Skill ID from aporto_discover_skills"),
    ...
    server.tool(
        "aporto_run_skill",
        "High-level Aporto skill runner. Use this as the default tool when the user asks you to perform a task. It discovers the skill when needed, selects a provider, executes it, stores a SkillRun, and returns a runId on every response. If status is needs_selection, show the choices to the user once and then call aporto_run_skill again with the chosen skillId. When status is succeeded, return the artifacts to the user. When status is running or waiting, automatically call aporto_get_skill_run every 30 seconds until status is succeeded or failed; do not ask the user to poll manually. When status is failed and error.retryable is true and error.code is not 'error_4xx', 'INSUFFICIENT_BALANCE', 'NO_ACTIVE_PROVIDER', or 'SKILL_NOT_FOUND', retry aporto_run_skill with the same sessionId (required for provider exclusion) up to 2 times before giving up.",
        {
        ...
        async ({ intent, params = {}, skillId, providerHint, waitForResult = true, maxWaitSeconds = DEFAULT_WAIT_SECONDS, sessionId }) => {
            try {
                ...
                return {
                    content: [{ type: "text" as const, text: `Run error: ${String(err)}` }],
                    isError: true,
                };
            }
        },
    ...
    server.tool(
        ...
        async ({ runId, waitForResult = true, maxWaitSeconds = DEFAULT_WAIT_SECONDS }) => {
            try {
                ...
                if (!result) {
                    return {
                        content: [{ type: "text" as const, text: "Skill run not found." }],
                        isError: true,
                    };
                }
                return {
                ...
                return {
                    content: [{ type: "text" as const, text: `Run status error: ${String(err)}` }],
                    isError: true,
                };
            }
        },
