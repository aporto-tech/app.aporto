...
import { newApiGetLogs } from './src/lib/newapi'
...
async function main() {
    ...
    for (const email of usersToFix) {
        const aportoUser = await prisma.user.findUnique({ where: { email }});
        if (!aportoUser) continue;
        
        const apiUser = data.data.find((u: any) => u.email === email);
        if (apiUser) {
            ...
            console.log(`❌ Could not find ${email} in NewAPI`);
        }
    }
}
