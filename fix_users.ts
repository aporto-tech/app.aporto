...
import { newApiCreateUser } from './src/lib/newapi';
const prisma = new PrismaClient();

// MUST use the production URL and token as the DB is on production and the previous test proved it
...
async function main() {
  ...
  for (const user of users) {
      ...
      const newApiUser = await newApiCreateUser({
          username,
          email: user.email,
          password: "12345678" // It's fine since Aporto uses its auth and syncs ID
      });
